    ...
    while ((match = linkRegex.exec(line)) !== null) {
      ...
      parts.push(
        <Link
          key={`${key}-lnk-${start}`}
          href={href}
          className="mx-1 inline-block rounded-full border border-blue-800/40 bg-blue-900/30 px-3 py-1 text-blue-100 no-underline transition-colors hover:bg-blue-900"
        >
          {label}
        </Link>
      );
