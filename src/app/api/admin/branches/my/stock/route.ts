...
export async function GET(request: Request) {
  try {
    ...
    if (userRole !== "Branch") {
      return NextResponse.json(
        { error: "This endpoint is only for branch users" },
        { status: 403 }
      );
    }
    ...
    return NextResponse.json({
      ...
      hasStock: productStock !== null && productStock.quantity !== null && productStock.quantity > 0,
    ...
    return NextResponse.json(
      { error: 'خطا در بررسی موجودی شعبه' },
      { status: 500 }
    );
  }
} 