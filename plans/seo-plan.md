...
**OVERALL: PARTIAL** — items 1, 2, 5 done; items 3, 4, 6 partially done.

| #   | Item                           | Status                                                                                                                                        |
| --- | ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Metadata Standardization       | ✅ DONE — `src/constants/meta.ts` + `metadata`/`generateMetadata` on ~20 pages                                                                |
| 2   | Structured Data (Schema.org)   | ✅ DONE — `Schema.tsx`, homepage `@graph`, FAQPage/BlogPosting/BreadcrumbList etc.                                                            |
| 3   | URL & Route SEO                | ⚠️ PARTIAL — `[slug]` routes + `sitemap.ts` + `robots.ts` exist; no hreflang/alternate-language; `sitemap.ts` hardcodes `https://farabak.net` |
| 4   | Content & Heading Optimization | ⚠️ PARTIAL — `src/helpers/contentHelper.ts` exists but is dead code (never imported); no `npm run seo:audit` script                           |
| 5   | Image SEO                      | ✅ DONE — `next/image` w/ width/height + blurDataURL + alt; no raw `<img>` on public pages                                                    |
| 6   | Internal Linking & Navigation  | ⚠️ PARTIAL — `InternalLink.tsx` is dead code; header/footer use `next/link` directly; footer WhatsApp link lacks `target="_blank"`            |

### Remaining

- Wire `contentHelper.ts` into pages/audit (or delete) + add `seo:audit` npm script.
- Wire `InternalLink.tsx` into header/footer/nav (or delete); fix footer WhatsApp rel/target.
- Add hreflang/alternates only if an English/transliterated variant is planned; parametrize sitemap base URL.
...
_Plan can be executed in parallel by subagents: one handles metadata & schema, another image SEO, another sitemap/robots._
